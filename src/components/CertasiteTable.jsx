...
export default function CertasiteTable() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/certasite')
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`)
      }
      const result = await response.json()
      console.log('Certasite data received:', result)
      ...
      console.error('Failed to fetch Certasite data:', err)
      setData([])
      setLoading(false)
    }
  }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/certasite/${id}`, { method: 'DELETE' })
      if (!response.ok) {
  ...
  const handleSave = async (formData) => {
    try {
      ...
      const url = formData.id 
        ? `http://localhost:5000/api/certasite/${formData.id}`
        : 'http://localhost:5000/api/certasite'
...
function CertasiteFormModal({ data, onClose, onSave }) {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={e => e.stopPropagation()}>
        <div className="modal-header">
          <h2>{data ? 'Edit' : 'Add'} Certasite Record</h2>
